...
std::vector<VertexId> get_unconnected_vertex_ids(
  ...
  std::vector<VertexId> unconnected_vertexes = {};
  for (const auto cur_vertex_id : vertex_ids_on_depth) {
    if (!graph.is_connected(vertex_id, cur_vertex_id)) {
      unconnected_vertexes.emplace_back(cur_vertex_id);
    }
  }

  return unconnected_vertexes;
}
...
void generate_green_edges(Graph& graph, std::mutex& graph_mutex) {
  ...
  const std::unordered_map<VertexId, std::unique_ptr<IVertex>>& vertexes =
      graph.get_vertexes();
...
void generate_yellow_edges(Graph& graph,
  ...
  for (GraphDepth current_depth = kGraphBaseDepth;
    ...
    const auto& vertexes = graph.vertexes_of_depth(current_depth);
          ...
          if (check_probability(prob)) {
            ...
            if (!unconnected_vertexes_ids.empty()) {
              const auto to_vertex_id =
                  get_random_vertex(unconnected_vertexes_ids);
              const std::lock_guard<std::mutex> guard(graph_mutex);
...
void generate_red_edges(Graph& graph,
  ...
  for (GraphDepth current_depth = kGraphBaseDepth;
    ...
    const auto& depth_vertexes = graph.vertexes_of_depth(current_depth);
    const auto& deeper_vertexes =
        graph.vertexes_of_depth(current_depth + kRedEdgesDiff);

    std::for_each(depth_vertexes.begin(), depth_vertexes.end(),
                  [&graph, &graph_mutex,
                   &deeper_vertexes](const VertexId from_vertex_id) {
                    const auto to_vertex_id =
                        get_random_vertex(deeper_vertexes);
                    if (check_probability(prob)) {
                      const std::lock_guard<std::mutex> guard(graph_mutex);
                      graph.add_edge(from_vertex_id, to_vertex_id);
                    }
