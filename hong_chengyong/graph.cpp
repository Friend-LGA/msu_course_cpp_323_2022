...
namespace uni_course_cpp
{
                           ...
                           const VertexId &to_vertex_id) const
  {
    ...
    if (from_vertex_id == to_vertex_id)
    {
      for (const auto &edge_id : connection_list_.at(from_vertex_id))
      {
        ...
        if (edge.from_vertex_id == from_vertex_id && edge.to_vertex_id == from_vertex_id)
        {
          return true;
        }
      }
    ...
    else
    {
      ...
      {
        const auto &edge = getEdge(edge_id);
        if (edge.from_vertex_id == to_vertex_id || edge.to_vertex_id == to_vertex_id)
        {
          return true;
        }
      }
                                         ...
                                         const VertexId &to_vertex_id) const
  {
    ...
    else if (std::abs(vertexDepth(to_vertex_id) - vertexDepth(from_vertex_id)) == 1)
    {
  ...
  void Graph::addEdge(const VertexId &from_vertex_id, const VertexId &to_vertex_id)
  {
    assert(hasVertex(from_vertex_id) && "Vertex1 index is out of range");
    assert(hasVertex(to_vertex_id) && "Vertex2 index is out of range");
  ...
  const std::vector<VertexId> &Graph::vertexIdsAtLayer(uni_course_cpp::Depth depth) const
  {
